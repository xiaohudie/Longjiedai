               ...
               <tbody>
                   <tr v-for="(item,index) in dataList">
                      <td>
                       ...
                       <td>
                           <span v-if="item.status==5">已退款</span>
                           <span v-else-if="item.status==1">等待处理</span>
                            <span v-else-if="item.status==2">处理中</span>
                             <span v-else-if="item.status==3">提现失败</span>
                              <span v-else-if="item.status==4">待到账</span>
                              <span v-else-if="item.status==6">已到账</span>
                       </td>
                       <td>
...
export default {
  ...
  methods: {
    getRechargelist:function() {
      var that = this;
      this.$api.get("/web-api/account/getPageCashRecord", null, function(res) {
        that.data = res.data;
    ...
    postpageIndex:function(index) {
      var inpagenum =index;
      this.$api.post(
        "/web-api/account/getPageCashRecord",
        {"pageIndex":inpagenum},
        function(res) {
          this.data = res.data;
